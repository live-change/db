...
class ObjectReader {
  ...
  async onChange(cb) {
    cb(await (await this.table).objectGet(this.id), null)
  }
  async get() {
...
class RangeReader {
  ...
  async onChange(cb) {
    ...
    for(let object of objects) cb(object, null)
  }
  onDelete(cb) {}
  async get() {
...
class QueryWriter {
  ...
  getResults() {
    return Array.from(this.results.entries()).sort((a,b)=>a[0]>b[0]?1:(a[0]<b[0]?-1:0) ).map(a => a[1])
  }
...
async function queryGet(database, code) {
  const reader = new QueryReader(database)
  const writer = new QueryWriter()
