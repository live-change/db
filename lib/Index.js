...
class TableReader {
  constructor(opLogReader, prefix, table) {
  ...
  async readOpLog(key) {
    ...
    this.opLogPromise = new Promise(async (resolve,reject) => {
      ...
      if(!this.opLog) this.opLog = (await this.table).opLog
...
class Index extends Table {
  constructor(database, name, code, config) {
  ...
  async startIndex() {
    const lastIndexOperations = this.opLog.rangeGet({ reverse: true, limit: 1 })
