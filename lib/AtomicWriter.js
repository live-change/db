...
const mutators = {
  ...
  addToSet(obj, { property, value }) {
    updateProperty(obj, property, v => [v||[]].filter(x => x!=value).concat(value))
  },
  deleteFromSet(obj, { property, value }) {
    updateProperty(obj, property, v => [v||[]].filter(x => x!=value))
  },
  add(obj, { property, value }) {
    updateProperty(obj, property, v => (v||0) + value)
  },
  merge(obj, { property, value }) {
    updateProperty(obj, property, v => ({ ...(v||{}), ...value }))
  }
}

function mutate(value, operations) {
  for(let operation of operations) {
    mutators[operation.op](value, operation)
  }
}
