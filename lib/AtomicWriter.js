    ...
    if(first) {
      if(this.writePromise) {
        ...
        this.updatePromise = this.writePromise.then(async written => {
          ...
          const value = mutate(JSON.parse(JSON.stringify(this.writeValue)) || { id: this.id }, this.operations)
          this.operations = []
        ...
        this.updatePromise = this.readPromise.then(async readed => {
          ...
          if(this.operations == []) return
          const value = mutate(JSON.parse(JSON.stringify(readed)) || { id: this.id }, this.operations)
          this.operations = []
          await this.put(value)
