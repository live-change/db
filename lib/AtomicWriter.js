...
const mutators = {
  ...
  addToSet(obj, { property, value }) {
    ...
    updateProperty(obj, property, v => [v||[]].filter(x => JSON.stringify(x) != valueStr).concat(value))
    return obj
  },
  deleteFromSet(obj, { property, value }) {
    const valueStr = JSON.stringify(value)
    updateProperty(obj, property, v => [v||[]].filter(x => JSON.stringify(x) != valueStr))
    return obj
  },
    ...
    if(first) {
      if(this.writePromise) {
        ...
        this.updatePromise = this.writePromise.then(async written => {
          ...
          const value = mutate(JSON.parse(JSON.stringify(this.writeValue)) || {}, this.operations)
          this.operations = []
        ...
        this.updatePromise = this.readPromise.then(async readed => {
          ...
          if(this.operations == []) return
          const value = mutate(JSON.parse(JSON.stringify(readed)) || {}, this.operations)
          this.operations = []
          await this.put(value)
